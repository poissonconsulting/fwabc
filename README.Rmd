...
`fwabc` uses the [`bcdata`](https://github.com/bcgov/bcdata) package to query the atlas remotely. It provides convenience for common queries and tools to facilitate working with the data.

Let's say we want to use the bcdata package to get the Fraser River stream-network polyline plus all its named tributaries with stream order > 5.

We could do:
```{r bcdata2, eval = FALSE}
# frasr river stream
...
1. that the record id for the 'stream-network' dataset is '92344413-8035-4c08-b996-65a9b3f62fca';
2. that any tributaries of the Fraser River have FWA_WATERSHED_CODE starting with "100-";
3. some CQL;
4. variable names in the 'stream-network' dataset.
