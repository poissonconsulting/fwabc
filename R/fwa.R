...
#' @param stream A vector of valid BlueLineKey and/or GnisName (see fwa_blue_line_key and fwa_gnis_name for reference).
#' @param
#' @return A sf object.
#' @examples
#' stream <- ps_fwa_stream(stream = c(354087611, "Kaslo River"))
#' @export
fwa_stream <- function(dsn = "~/Poisson/Data/spatial/fwa/gdb/FWA_BC.gdb", stream = "Kaslo River", recursive = FALSE) {
  check_dsn(dsn, layer = "FWA_ROUTES_SP")
  check_stream(stream)

  blk <- gnis_to_blk(stream)
  st_read(dsn = dsn, layer = "FWA_ROUTES_SP", query = or_sql(blk) %>% snip %>% select_sql)
}
